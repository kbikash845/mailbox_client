...
export default function SentMail() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://clone-21efb-default-rtdb.firebaseio.com/sent/user/${updatedEmail}.json`
        );
  ...
  const hideBtnHandler = async (index, emailId) => {
    ...
    try {
      const response = await fetch(
        `https://clone-21efb-default-rtdb.firebaseio.com/sent/user/${updatedEmail}/${emailId}.json`,
        {
          method: "PUT",
          body: JSON.stringify({
            id: emailId,
